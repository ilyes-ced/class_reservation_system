...
if(isset($_POST['refuse'])){
  $req = $bdd->query('DELETE  FROM  demands WHERE id='.$_POST['refuse']);
  unset($_POST['refuse']);
}
...
if(isset($_POST['accept'])){
  ...
  $req2 = $bdd->query('SELECT * FROM demands where id='.$_POST['accept']);
  while($gg=$req2->fetch()){
    $id=$gg['id'];
    $cell=$gg['cell'];
    $sender=$gg['sender'];
  ...
  if($cell[0]=='c'){
    ...
    $req->execute(array( 
     ...
     // header("Refresh:0");
     $req = $bdd->query('DELETE  FROM  demands WHERE id='.$_POST['accept']);
...
$dev = explode('|',$dd);
if($dev[1]!='none'){$kk.='<a class="to_be_colored d2">'.$dev[1]."</a>";}
if($dev[2]!='none'){$kk.='<a class="to_be_colored d33">'.$dev[2]."</a><br/>";}

if($dev[0]!='none'){$kk.='<a class="to_be_colored d">'.$dev[0]."</a><br/>";}
if($dev[3]!='none'){$kk.='<a class="to_be_colored d4">'.$dev[3]."</a>";}
